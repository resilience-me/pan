...
async function fetchAccountInfo(address, isMetamask) {
	...
	if(data.bitpeople.proofOfUniqueHuman) {
		...
		if(isMetamask) {
			...
			responseDisplay.innerHTML += `<input type="text" id="courtAddressInput" placeholder='Enter "court" address here' size="42" oninput="validateCourtAddressInput()">`;
			responseDisplay.innerHTML += ' <button id="judgeButton" onclick="judge(document.getElementById(\'courtAddressInput\').value)" disabled>Judge</button>';	
		} else {
